...
> **2026-08-01 资源模型 / 批量 / 账号模板轮次(已完成)**:用户指出「角色资源需要参照 nacos 的资源设置,
> 不然你这个权限就是空的」—— 确实,之前授权只会写死 `<ns>:*:*`,Nacos 的资源模型丢了一半。
> **A 按 Nacos 资源模型拼串**(`build_resource`,对齐源码 `NacosRoleServiceImpl#joinResource`):
> `<namespaceId>:<group>:<type>/<name>`,分隔符 `:`、通配 `*`;public 首段留空(`:*:*`);
> **类型为「全部」时第三段整体塌缩成 `*`** —— 授权判定是把存储的资源串按 `*`→`.*` 变正则整串匹配,
> 写成 `*/*` 永远匹配不上,这就是「权限是空的」的根因。类型只放行 `config|naming|*`(服务端不校验,
> 写错不会报错但永不生效,必须本地挡)。授权弹窗加「限定资源范围」折叠项 + 实时资源串预览。
> **B 批量授权**:`POST /grant/batch`,一个账号 × 多个命名空间一次授完,角色只建一次,逐个报结果;
> UI 命名空间改多选 + 全选/清空。
> **C 账号模板**:新表 `nacos_account_templates`(账号 + 默认口令 + 要授的命名空间),
> `POST /clusters/{id}/accounts/apply` 一键落到任意集群 —— 缺失的按模板口令新建,
> **已存在的账号不会被重置口令**,只补授权;支持 dry_run。Nacos 总览页新增「账号模板」Tab。
> **D 幂等**(重跑是这套东西的前提):Nacos 的 permissions 表对 (role,resource,action) 有唯一约束,
> 重复插入会报错。下发前先拉一次现状,已有的跳过并标 `exists`,不把重跑变成一堆失败。
> 验证:`cargo test -p opsctl-server` **88 全绿**(新增 5:资源串七种形状 + 类型白名单 + 批量授权 +
> 账号模板落地 + 幂等重跑);Playwright 实测:资源预览随 group/类型/名称实时变化并与后端逐字一致
> (`:DEFAULT_GROUP:config/app.yaml`)、批量授两个命名空间、账号模板试运行→执行→重跑显示
> 「已存在,未改口令」;重复批量授权返回 `exists` 且远端不新增行。
